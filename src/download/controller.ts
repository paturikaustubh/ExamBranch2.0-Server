...
async function downloadTable(
    ...
    try {
        [result, fields] = (await dbQueryWithFields(query)) as [
            any,
            FieldInfo[]
        ]
    } catch (err) {
...
const tables: {
    ...
    supplereport: {
        query: `select subCode as Code, subName as Subject,count(*) as Total from paidsupply p group by subCode,subName `,
        ordering: ' ORDER BY count(*) desc, subCode ',
